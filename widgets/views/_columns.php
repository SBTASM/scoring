...
foreach ($columnSelector as $value => $label) {
    if (in_array(urlencode($value), $hiddenColumns)) {
        $checked = in_array(urlencode($value), $selectedColumns);
        echo Html::checkbox('export_columns_selector[]', $checked, ['data-key' => urlencode($value), 'style' => 'display:none']);
        unset($columnSelector[$value]);
    }
    if (in_array(urlencode($value), $noExportColumns)) {
        unset($columnSelector[$value]);
    }
}
...
foreach ($columnSelector as $value => $label) {
    ...
    echo '<li><div class="checkbox">' . $labelTag .
        Html::checkbox('export_columns_selector[]', $checked, ['data-key' => urldecode($value), 'disabled' => $disabled]) .
        "\n" . $label . '</label></div></li>';
}
echo Html::endTag('ul');
